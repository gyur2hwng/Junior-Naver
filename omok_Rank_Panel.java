...
public class omok_Rank_Panel extends JPanel implements ActionListener {
	JLabel mainLb;
	JLabel[] rankLb;
	
	JButton backMain;
	
	omok_Rank_Panel()  throws FontFormatException, IOException{
		setLayout(null);
		...
		{
			if(omok_Rank.instance.rank[i][0].equals("blank1") || omok_Rank.instance.rank[i][0].equals("blank2") || omok_Rank.instance.rank[i][0].equals("blank3"))
			{
				rankLb[i].setText("[" + (i+1) + "등] ");
			}
			else
			{
				rankLb[i].setText("[" + (i+1) + "등] " + omok_Rank.instance.rank[i][0] + " : " + omok_Rank.instance.rank[i][1]);
			}
		}
