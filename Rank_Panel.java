...
public class Rank_Panel extends JPanel implements ActionListener {
	...
	Rank_Panel() throws FontFormatException, IOException
	{
		setLayout(null);
		...
		{
			if(Rank.instance.rank[i][0].equals("blank1") || Rank.instance.rank[i][0].equals("blank2") || Rank.instance.rank[i][0].equals("blank3"))
			{
				rankLb[i].setText("[" + (i+1) + "등] ");
			}
			else
			{
				rankLb[i].setText("[" + (i+1) + "등] " + Rank.instance.rank[i][0] + " : " + Rank.instance.rank[i][1]);
			}
		}
	}
