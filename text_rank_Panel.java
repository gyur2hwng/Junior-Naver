...
public class text_rank_Panel extends JPanel implements ActionListener{	
	JButton back_main;
	
	JLabel mainLb;
	JLabel [] rankLb;
	
	text_rank_Panel() throws FontFormatException, IOException {
		setLayout(null);
		...
		for (int i=0;i<text_Rank.instance.rank_text.length;i++) {
			if (text_Rank.instance.rank_text[i][0].equals("blank1") ||
					text_Rank.instance.rank_text[i][0].equals("blank2") ||
					text_Rank.instance.rank_text[i][0].equals("blank3")) {
				rankLb[i].setText("[" + (i+1) + "등] ");
			}
			else {
				rankLb[i].setText("[" + (i+1) + "등] " + text_Rank.instance.rank_text[i][0] + " : " + text_Rank.instance.rank_text[i][1]);
			}
		}
	}
	...
	public void actionPerformed(ActionEvent e) {	
		if (e.getSource() == back_main) {
			try {
				_WordGame.frame.setContentPane(new text_stage1_Panel());
