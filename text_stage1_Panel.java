...
public class text_stage1_Panel extends JPanel implements ActionListener {
	...
	text_StopWatch sw; //스탑와치
	
	ImageIcon img = new ImageIcon();
	JLabel bottom;
	int check = 0; //리셋 가능 여부를 확인하기 위한 체크
	
	text_stage1_Panel() throws FontFormatException, IOException {
		setLayout(null);
	...
	public void setGamePanel() throws FontFormatException, IOException {
		...
		inputLb.setBounds(430, 200, 400, 100);
	...
	public void actionPerformed(ActionEvent e) {
		...
		//랭킹 버튼 눌렀을 때
			...
			try {
				_WordGame.frame.setContentPane(new text_rank_Panel());
		...
		//게임 버튼 눌렀을 때
			...
			// in_tf.setFocusTraversalKeysEnabled(false);
			setInit();
			sw = new text_StopWatch();
			sw.start();
		}
		//리셋 버튼 눌렀을 때
		...
		//확인 버튼 눌렀을 때
			...
			if(in_tf.getText().equals(inputLb.getText())) {
				WIN_COUNT++;
				if (WIN_COUNT < 10) {
					countLb.setText("WIN_COUNT = " + WIN_COUNT);
					...
					sw.play = false;
					sw.interrupt();
					JOptionPane.showMessageDialog(null, "총 소요시간은" + sw.timeText + "초 입니다.", "게임 종료",
							JOptionPane.INFORMATION_MESSAGE);
					watchLb.setText("");
					inputLb.setText("");
					in_tf.setText("");
					text_Rank.instance.text_setInfo(FileManager.LOG,
							sw.timeText);
					FileManager.instance.saveTextStage1RankData();
					in_tf.requestFocus();
				}
			}
		}
